...
const ERROR_KEYS: Record<string, string> = {
  user_not_found: "collab.error.userNotFound",
  already_owner: "collab.error.alreadyOwner",
};
...
export function CollaboratorsPanel({ wikiId }: { wikiId: string }) {
  ...
  async function addCollaborator(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(ERROR_KEYS[data.error] ? t(ERROR_KEYS[data.error]) : t("collab.error.generic"));
      return;
    }
    setEmail("");
    refresh();
  ...
  return (
    <div className="mt-2 rounded-md border border-neutral-200 p-3 text-sm dark:border-neutral-800">
      <h3 className="mb-2 font-medium">{t("collab.title")}</h3>

      {collaborators === null && <p className="text-neutral-400">{t("collab.loading")}</p>}
      {collaborators?.length === 0 && <p className="text-neutral-400">{t("collab.none")}</p>}
      ...
      <form onSubmit={addCollaborator} className="flex gap-1">
        <input
          ...
          placeholder={t("collab.addPlaceholder")}
